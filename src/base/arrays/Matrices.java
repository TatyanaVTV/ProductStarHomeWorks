    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            if (other.items.length != items.length) {
                throw new IllegalArgumentException("Векторы должны иметь одинаковый размер!");
            }
    ...
    public static class Matrix {
        ...
        public Matrix multiply(Matrix other) {
            if (nCols != other.nRows) {
                throw new IllegalArgumentException("Количество строк другой матрицы должно быть равно количеству" +
                        "столбцов текущей матрицы для умножения.");
            }
            ...
            for (int iRow = 0; iRow < nRows; iRow++) {
                for (int iCol = 0; iCol < other.nCols; iCol++) {
                    for (int k = 0; k < nCols; k++) {
                        result.rows[iRow][iCol] += this.rows[iRow][k] * other.rows[k][iCol];
                    }
                }
            }
            return result;
        ...
        public Matrix transpose() {
            ...
            for (int iCol = 0; iCol < nRows; iCol++) {
                for (int iRow = 0; iRow < nCols; iRow++) {
                    result.rows[iRow][iCol] = this.rows[iCol][iRow];
                }
            }
            return result;
        }
